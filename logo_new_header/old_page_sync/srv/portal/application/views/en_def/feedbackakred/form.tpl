...
function SelAnons( id )
{
        $.ajax({
         type: "POST",
         url: "/en/feedbackakred/eventlist/"+id,
         data: {"reque":id},
...
<form action="" method="post">
    <div class="accreditation-form">
        ...
        <div class="accreditation-form__heading">
                        Fill out the accreditation form
        </div>
...
{*        <span class="warn">{$errors.eventname}</span>*}
        ...
        <div class="form-group capchure">
            <label>{$i18n.captcha}</label>
            <div>
                {$captcha}
            </div>
            <input class="form-control" type="text" name="captcha" value="" id="captcha"/>
             <span class="warn">{$errors.captcha}</span>
        </div>
        <div class="form-group">
             <input type="submit" class="sendcomm btn btn-primary" value="Send" onclick="sendMsg(); return false;"/>
        </div>
