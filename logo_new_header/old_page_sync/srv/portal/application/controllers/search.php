...
class Search_Controller extends Front_Controller {
    ...
    public function __construct() {
      //  Kohana::show_404();
        ...
        if (empty($_GET)) {
            $this->timestamp = time();
            setcookie("ht", $this->timestamp, time()+120);  /* expire in 2 min */
        }
        isset($_COOKIE["ht"]) ? $this->cookie_set = true : $this->cookie_set = false;


      //  var_dump($this->cookie_set); die("application/controllers/search.php:26");
...
//var_dump($this->cookie_set); die("application/controllers/search.php:49");

    //  var_dump($this->submit); die("application/controllers/search.php:35");
