  ...
  public function getEncoded($name)
  {
    ...
    foreach ($encoded_value as $key => $row)
    {
      ...
      if (false !== $p = strpos($encoded_value[$key], $this->LE))
      {
         ...
         * e (PREG_REPLACE_EVAL) Внимание Эта функциональность считается УСТАРЕВШЕЙ с PHP 5.5.0, и УДАЛЕНА в PHP 7.0.0.
         Для предотвращения уязвимости с удаленным исполнением кода следует использовать функцию preg_replace_callback():  rewrite by mirs 21*/
...
/*      $source_content = preg_replace_callback($search, 
 ...
 //       $encoded_value[$key] = preg_replace("/<([^>]+)>", function ($m) {return $cb;}, $encoded_value[$key]);
