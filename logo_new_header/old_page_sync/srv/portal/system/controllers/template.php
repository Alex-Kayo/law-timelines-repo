...
//        print_r($this->site_title.'ooo');
        ...
        if ( $data_point = $this->input->get('date_point_my')) {
                $this->date_pieces = explode(".", $data_point); // mirs новий сайт 19.09.2019


            }
//   mirs      прередає в пошукову форму назву обраної категорії
        if ( $topic_id = intval($this->input->get('search_topic_id', 0, true))){
            $topic_name = $this->db->query("select t_name from topics where t_id = {$topic_id}")->current()->t_name;
        }
        else  $topic_name = null;
//       var_dump($topic_name); die();

        $this->search_params = array(
            'date_point_my' => $data_point,
            'search_topic_id' =>  $topic_id,
            'search_topic_name' => $topic_name,
//            'search_day' => intval($this->date_pieces[0]),
            'search_month' => intval($this->date_pieces[0]),
            'search_year' => intval($this->date_pieces[1]),
