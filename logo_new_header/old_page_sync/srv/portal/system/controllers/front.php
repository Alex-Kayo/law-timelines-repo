...
class Front_Controller extends Template_Controller {
    ...
    /*
     * мітка сабсайту фінонсово_господарськ.
     */
    protected $is_gospfin = FALSE;

    /*
     * мітка сабсайту НАТО.
     */
   // protected $is_nato_subsite = FALSE;
   ...
   // private $cache_main_page_life = 3600;
	private $cache_main_page_life = 120;
    ...
    public function __construct() {
       ...
       $ip_back = Kohana::config('config.back_ip');
       // var_dump($ip_back); die();
      // header("X-Frame-Options: SAMEORIGIN");
        header("Content-Security-Policy: frame-ancestors 'self' $ip_back"); // mirs для відображення в адмінці
        
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        $this->mainPageBlockBuilder();

        // Налаштування хедерів сабсайтів (показ меню сабсайтів)

        /*if(in_array($this->current_node->t_seo_name, Kohana::config('all_subsite_seos.nato_seos_list')))
            $this->is_nato_subsite = TRUE;*/

        if(in_array($this->current_node->t_seo_name, Kohana::config('all_subsite_seos.gospfin_seos_list')))
            $this->is_gospfin = TRUE;
...
//    var_dump($this->current_node->t_seo_name);    die();
        ...
        // читаємо конфіг файла з переліком seos для фільтрації
        
        foreach (Kohana::config('hide_mainpage_seos.seos_list') as $seos){
            
          $topid[] =  $this->getTopicID($seos);
            
        }
        
        $topid_s = implode($topid, ','); //comma separated string
        
 //       var_dump($topid_s); die();
//        var_dump(Kohana::config('hide_mainpage_seos.seos_list')); die();
        
        $this->view->logo_news = $this->getLogoNewsData();
        
        
        $this->view->logo_news_mp = $this->getLogoNewsMpData($topid_s);
        
       // var_dump($this->view->logo_news_mp); die();
 ...
 //       $this->view->is_plenar = 1;
    ...
    //    var_dump($this->current_node->t_seo_name);        die();
        ...
        if ($this->class == 'main_page') {
            ...
            $this->view->is_gospfin_subsite = $this->is_gospfin; //mirs Вивід мітки госпфін сабсійту у шаблон
    ...
    public function show() {
        ...
        $this->view->site_title = strip_tags( $this->view->item->i_name) . ' - ' . $this->site_title; //mirs Вивід інфо у заголовок сайту
//        var_dump($this->view->site_title); die("system/controllers/front.php:290");
//        if(in_array($this->current_node->t_seo_name, Kohana::config('nato_site_seos.seos_list')))

 //       $this->view->is_nato_subsite = $this->is_nato_subsite;

//        if(in_array($this->current_node->t_seo_name, Kohana::config('gospfin_site_seos.seos_list')))
        $this->view->is_gospfin = $this->is_gospfin;
        ...
        if (isset($this->item_id[1])) {
            ...
            reset($attachments);
            //mirs в php7.1 видає помилку Only variables should be passed by reference
          //  $val = array_shift(array_values($attachments));
            ////////////А так проходить////////////////
            $array = array_values($attachments);
...
// mirs внесено зміни у $sqlStr рядок --AND l_langnick = '$langNick' для не укр версій
        $sqlStr = "
    		...
    		AND (
                ...
                "AND i_date_create " . ($itemsType === 'preview' ? '> (-0.125) +' : '<=') . " sysdate				
    		) i_all
    			INNER JOIN months m ON to_number(to_char(i_all.i_date_create,'mm'))=m.m_number AND l_langnick = '$langNick'
    		ORDER BY i_date_create " . ($itemsType === 'preview' ? 'ASC' : 'DESC') . ",  i_id DESC)
...
//    protected function getTopic($uniqParam){
            ...
            if ($langConfig = $this->getLangConfig()) {
                if ($rootNode = $this->getTopic('news')) {
                  //  $result = $this->makeMainFind('news', array($rootNode->t_left_key, $rootNode->t_right_key), $this->siteLang, $this->logo_news, 'i.i_logoname IS NOT NULL AND i.t_id IN (select t_id from topics WHERE T_RSSLIST IS NULL)' . $this->betweenDateConditions); //mirs
                   $result = $this->makeMainFind('news',array($rootNode->t_left_key, $rootNode->t_right_key), $this->siteLang, $this->logo_news, 'i.i_logoname IS NOT NULL AND i.t_id IN (select t_id from topics WHERE T_RSSLIST IS NULL) AND i.t_id !=' .$langConfig->l_top_news_topic . ' '.$this->betweenDateConditions);//javir не дублює топ-новини
   ...
   /**
	 * Повертає радки запиту logo news за виключенням T_ID переданих у параметрі.
	 *
	 * @param   string  comma separated T_IDs
	 * @return  class rows
	 */
    
    private function getLogoNewsMpData($NoShowTid) {
        // $result = false;
        $cacheID = "main_page_logo_mp_news" . $this->siteLang . md5($this->betweenDateConditions);
        if (!$result = $this->cache->get($cacheID)) {
            if ($langConfig = $this->getLangConfig()) {
                if ($rootNode = $this->getTopic('news')) {
                  //  $result = $this->makeMainFind('news', array($rootNode->t_left_key, $rootNode->t_right_key), $this->siteLang, $this->logo_news, 'i.i_logoname IS NOT NULL AND i.t_id IN (select t_id from topics WHERE T_RSSLIST IS NULL)' . $this->betweenDateConditions); //mirs
                   $result = $this->makeMainFind('news',array($rootNode->t_left_key, $rootNode->t_right_key), $this->siteLang, $this->logo_news, 'i.i_logoname IS NOT NULL AND i.t_id IN (select t_id from topics WHERE T_RSSLIST IS NULL) AND i.t_id not in (' .$NoShowTid . ') '.$this->betweenDateConditions);//javir не дублює топ-новини
   ...
   //     var_dump($result); die();
    ...
    public function mainPageBlockBuilder() {
       ...
       //     var_dump($this->view->panel_config);die;
    ...
    protected function getItemsSelTopics($className, $topic_id, $limit, $offset) {
        ...
        $result = array();        
// REGEXP_REPLACE(i_link, 'ns_golos', 'ns_golos_23') as i_link, mirs 
        $this->db->select("i_id,
                    i_name,
                    i_anons,
                    i_author,
                ...
                /*Кількість атачментів*/
                    "CASE 
                     ...
                     (select count(1) from items_attachments ia where  ia.i_id = i.i_id)
                      ...
                      0
                      END AS cntat,
                    '" . substr($this->view->base_url, $this->view->lang_prefix == '/en' ? 4 : 1, strlen($this->view->base_url) - 2) . "' AS i_categories") // mirs 23.10.2023 виправлення помилки у лінку /en/en/
                ->from("items i")
                ...
                ->where("i_status", 1)
                ->where("i_private", 0) /* mirs використовуєм для блокування показу у листі*/
    ...
    protected function getItemsSelClass($className, $limit, $offset) {
	    ...
	    // Не виводити пагінатор якщо показ усіх анонсів
            ...
            //($className === "preview" ? $limit = 100 : $offset = 0);
            if ($className === "preview")  $limit = 100;
            //============================================================ 
        ...
        $result['items'] = $this->db->query("
            ...
            FROM ( 
                SELECT * FROM (SELECT rownum as linenum, p_query.* FROM (
                    SELECT i_id, i_name, i_anons, i_link, i_logoname, i_author, t_id, is_video, CASE WHEN i_logoname IS NULL THEN 1 ELSE 0 END AS logo_null,". 
                /*Кількість атачментів*/
                    "CASE 
                     WHEN i.i_type = 'documents' then
                     (select count(1) from items_attachments ia where  ia.i_id = i.i_id)
...
//            
  ...
  //  var_dump($result['atcount']);         
        ...
        // Не виводити пагінатор якщо показ Усіх анонсів
            ($className === "preview" ? $result['total_count']=1 : $result['total_count'] = $this->db->query("SELECT count(1) cnt ". $baseCondition)->current()->cnt);
            //============================================================ 
