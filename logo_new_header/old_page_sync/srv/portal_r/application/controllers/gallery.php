...
//        $this->view->categories = $this->getCategories('gallery');
    ...
    public function show() {
        ...
        $this->search_params['search_day'] = $this->input->post('search_day', null, true);
        $this->search_params['search_month'] = $this->input->post('search_month', null, true);
        $this->search_params['search_year'] = $this->input->post('search_year', null, true);      
    //    var_dump($this->search_params);
        ...
        $this->db->select("CASE WHEN ia.ia_id IS NOT NULL THEN to_char(ia.ia_id)||'.'||ia.ia_type else '' END AS fileName, to_char(ia.i_id)||'_'||to_char(ia.ia_id) as foto_id, ia.ia_name, ia.ia_description, ia_author, ia_date_create")
//mirs додано ia_date_create 
        ...
        if ($this->search_params['search_day'] > 0) {
            $this->db->where("to_char(ia_date_create,'dd') = '" . str_pad($this->search_params['search_day'], 2, '0', STR_PAD_LEFT) . "'");
        }
        ...
        foreach ($this->db->get()->result() as $row) {
            $arrResult['items'][] = array('filename' => $row->filename, 'foto_id' => $row->foto_id,
                'ia_name'=> $row->ia_name, 'ia_description' => $row->ia_description, 'ia_author' => $row->ia_author, 'ia_date_create'=>$row->ia_date_create); //mirs додано ia_date_create для виводу дати створення фото в sel_foto.tpl
        }
   //  var_dump($this->search_params);        die();
 ...
 //       var_dump($this->db->last_query());        die();
    ...
    private function listGallery($idGallery = null) {
        ...
        if ($this->search_params['search_day'] > 0) {
            $attachmentsConditions[] = "to_char(ia_date_create,'dd') = '" . str_pad($this->search_params['search_day'], 2, '0', STR_PAD_LEFT) . "'";
        }
        if ($this->search_params['search_month'] > 0) {
...
order by FOTO_CNT desc")->as_array();    // mirs сортуємо по кількості фото в альбомаі                 
 ...
 //           var_dump($this->db->last_query()); die();
    ...
    private function selFoto($selIDFoto) {
        ...
        $this->view->sel_foto = $this->db->query("SELECT
         ...
         $this->view->site_title =  'Фото матеріали - '.strip_tags($this->current_node->t_name.' - '.$this->view->sel_foto->ia_name).' - '.$this->site_title; //mirs Вивід інфо у заголовок сайту
